...
private:
    void GNSSCB(const sensor_msgs::NavSatFixConstPtr &msg) {
        ...
        utm_converter::Wgs84 wgs84;
        wgs84.alt = msg->altitude;
        wgs84.lat = msg->latitude;
        wgs84.lon = msg->longitude;
        utm_converter::UtmCoor utm;
        utm = utm_converter::lla2utm(wgs84);
        ...
        if (distance > 0.1) {
            ...
            if (!firstYawInit) {
                firstYawInit = true;
                gtools.lla_origin_ << msg->latitude, msg->longitude, msg->altitude;
                origin_utm = utm_converter::lla2utm(wgs84);
            }
        ...
        // ----------------- 1. use utm -----------------------
        odom_msg.pose.pose.position.x = utm.x - origin_utm.x;
        odom_msg.pose.pose.position.y = utm.y - origin_utm.y;
        odom_msg.pose.pose.position.z = msg->altitude - origin_utm.z;
        ...
        // ----------------- 1. use utm -----------------------
        pose.pose.position.x = utm.x - origin_utm.x;
        pose.pose.position.y = utm.y - origin_utm.y;
        pose.pose.position.z = msg->altitude - origin_utm.z;
    ...
    utm_converter::UtmCoor origin_utm;
