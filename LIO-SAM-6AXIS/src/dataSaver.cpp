...
void DataSaver::saveOriginGPS(Eigen::Vector3d gps_point) {
    std::fstream originStream(save_directory + "origin.yaml", std::fstream::out);
    originStream.precision(15);
    originStream << "sensing:\n";
    originStream << "   gnss:\n";
    originStream << "      gnss_poser: {" << "origin_alt: " << gps_point[2] << ", origin_lat: " << gps_point[0] << ", origin_lon: " << gps_point[1] << "}";
    originStream.close();
}
...
void DataSaver::savePointCloudMap(std::vector<nav_msgs::Odometry> allOdometryVec,
    ...
    if (!globalmap->empty()) {
        ...
        try {
            pcl::io::savePCDFileBinary(save_directory + "global_map_lidar.pcd", *globalmap);
        ...
        if (use_imu_frame) {
            ...
            try {
                pcl::io::savePCDFileBinary(save_directory + "globalmap_imu.pcd", *globalmap);
                cout << "current scan saved to : " << save_directory << ", " << globalmap->points.size() << endl;
            } catch (std::exception e) {
                ROS_ERROR("SAVE PCD ERROR", globalmap->points.size());
            }
...
void DataSaver::savePointCloudMap(pcl::PointCloud<PointT> cloud_ptr) {
    ...
    pcl::io::savePCDFileBinary(save_directory + "globalmap_lidar_feature.pcd", cloud_ptr);
}
